...
<div class="items-left justify-left my-5 flex flex-col p-5 bg-gray-200 rounded-lg shadow-lg min-w-72">
	<label for="progress" class="font-quicksand text-lg font-semibold text-gray-700">Progress</label>
	<progress id="progress" value={percentComplete} max="100" class="mb-3 w-full">{percentComplete}%</progress>
	<input type="range" bind:value={duration} class="form-range w-full" min="0" max="100" />
	<hr class="my-3" />
	<p class="font-quicksand text-lg font-semibold text-gray-700 px-2 mx-2">Duration: {duration} seconds</p>
	<p class="font-quicksand text-lg font-semibold text-gray-700 px-2 mx-2">Elapsed Time: {elapsedTime} seconds</p>
	<button
		onclick={() => resetTimer()}
